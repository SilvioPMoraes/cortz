...
export async function GET(req: Request) {
  try {
    ...
    const tipo = url.searchParams.get("tipo") // Parâmetro 'tipo' (T ou P)
    const id = url.searchParams.get("id") // Parâmetro 'id'

    let sql = "SELECT * FROM servico"

    // Se o parâmetro 'id' for passado, fazer a consulta apenas para esse id
    if (id) {
      sql += ` WHERE id_barbearia = $1`
      const result = await db.query(sql, [id])
      //console.log(sql)
      //console.log(NextResponse.json(result.rows))
      return NextResponse.json(result.rows) // Retorna apenas o item com o ID fornecido
    } else {
      // Se não houver 'id', consulta todos os registros
      if (tipo === "T") {
        sql += ` LIMIT 800` // Retorna até 800 itens, caso o tipo seja "T"
      } else {
        sql += ` LIMIT 3` // Retorna até 3 itens, caso o tipo seja outro (ou padrão)
      }
      const result = await db.query(sql)
      return NextResponse.json(result.rows) // Retorna todos os itens (limitados)
    }
